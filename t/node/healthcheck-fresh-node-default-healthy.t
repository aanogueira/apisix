...
--- config
    location /t {
        content_by_lua_block {
            ...
            -- timer_create_checker runs on its own timer.every(1, ...), anchored to
            -- worker start rather than to this call, so up to ~1s can elapse before
            -- it picks up the waiting_pool entry and builds the checker; the checker's
            -- own active.healthy.interval (1s) only starts counting down from there.
            -- Worst case is just under 2s before the first probe fires -- padded well
            -- past that since this suite runs several nginx restarts back-to-back and
            -- CPU contention under that load measurably slows timer/probe delivery
            -- (the qoverflow-trial.sh settle-time investigation hit the same effect).
            ngx.sleep(4.5)
